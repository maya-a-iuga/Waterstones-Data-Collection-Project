...
class Scraper():
    ...
    def _bypass_cookies(self):
        ...
        accept_cookies_button = self.driver.find_element('xpath', '//*[@id="onetrust-accept-btn-handler"]')
    ...
    def _scraper_flags(self):
        ...
        parser = argparse.ArgumentParser(description = "This class will have three possible flags")
        
        parser.add_argument(
            '--category', 
            const = self.category_flag, 
            choices = ['f', 'c', 's', 'g', 'n'],
            nargs = '?',
            help  ='This flag specifies the book category, the user wants to scrape from',
            required = True
            )
        # flag = scrape from individual subcategories, no flag = scrape across them 
        parser.add_argument(
            '--subcategory', 
            action = self.subcategory_flag, 
            help = 'Flag specifies whether to scrape across book subcategories or pool them together')
        parser.add_argument(
            '--headless', 
            action = self.headless_flag,
            help = 'Flag specifies whether to run scraper in headless mode or not')
